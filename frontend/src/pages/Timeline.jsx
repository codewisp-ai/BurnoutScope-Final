...
/** Generate last-N-days array of date strings, oldest first */
...
/** Derive per-day risk level from commit count vs average */
...
/** Detect spike/crash days from weekly data */
function buildSpikeSet(dailyActivity) {
  ...
  counts.forEach((c, i) => {
    ...
    // crash = day after spike with < 50% commits
...
/** Approximate meeting hours per day from calendarData aggregate */
function estimateDailyMeetingHours(calendarData, dateRange) {
  if (!calendarData || !calendarData.totalMeetingHours) return {};
  // Spread total hours across working days proportionally (approximation)
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-6 animate-[fadeIn_0.25s_ease-out] relative">
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <button
          onClick={onClose}
          className="w-7 h-7 rounded-lg bg-white/8 border border-white/10 flex items-center justify-center text-white/40 hover:text-white/80 hover:bg-white/12 transition-all"
        >
          ✕
        </button>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        <span className={`text-xs px-2.5 py-1 rounded-full border font-mono ${cfg.color} ${cfg.bg} ${cfg.border}`}>
          {day.risk} Risk
        </span>
        {day.isWeekend && (
          <span className="text-xs px-2.5 py-1 rounded-full border font-mono text-purple-300 bg-purple-500/10 border-purple-500/30">
            Weekend Work
          </span>
        )}
        {day.isSpike && (
          <span className="text-xs px-2.5 py-1 rounded-full border font-mono text-orange-300 bg-orange-500/10 border-orange-500/30">
            🔺 Workload Spike
          </span>
        )}
        {day.isCrash && (
          <span className="text-xs px-2.5 py-1 rounded-full border font-mono text-red-300 bg-red-500/10 border-red-500/30">
            📉 Post-Spike Crash
          </span>
        )}
      </div>
      ...
      {day.commits === 0 && (
        <p className="text-xs text-white/30 font-mono italic">No commit activity on this day.</p>
      )}
    </div>
  );
}
...
export default function Timeline() {
  ...
  // Data from Dashboard via router state
  ...
  const allDays = useMemo(() => {
    ...
    return dateRange.map(date => {
      ...
      return {
        date,
        commits,
        meetingHours: meetingMap[date] || 0,
        isWeekend,
        isSpike:  spikeDates.has(date),
        isCrash:  crashDates.has(date),
        risk:     dayRisk(commits, avg),
      };
    });
  ...
  return (
    <div className="min-h-screen bg-[#080a0e] text-white font-['Syne',sans-serif] relative overflow-x-hidden print:bg-white print:text-black">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 md:px-6 py-10" ref={printRef}>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1 rounded-xl border border-white/10 bg-white/4 p-1">
              <button
                ...
                className="w-7 h-7 rounded-lg flex items-center justify-center text-white/40 hover:text-white hover:bg-white/8 transition-all text-sm font-mono"
                title="Zoom out"
              >
                −
              </button>
              <span className="text-xs font-mono text-white/30 px-1 w-10 text-center">
                {Math.round(zoom * 100)}%
              </span>
              <button
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-400/10 border border-amber-400/25 text-amber-400 text-xs font-mono hover:bg-amber-400/20 hover:border-amber-400/40 transition-all duration-200"
            >
        ...
        <div className="mb-6 animate-[fadeIn_0.4s_ease-out]">
          <StatStrip days={allDays} />
        </div>

        {/* ── Calendar notice ──────────────────────────────────────────────────── */}
        {!calendarData && (
          <div className="mb-5 rounded-xl border border-sky-500/20 bg-sky-500/5 px-4 py-3 flex items-center gap-3 no-print">
            ...
            <p className="text-xs text-sky-300/80 font-mono">
              No calendar data — showing GitHub commits only. Upload a CSV or ICS on the Dashboard to include meeting data.
            </p>
          </div>
        )}
        ...
        {/* ── Main chart card ─────────────────────────────────────────────────── */}
        <div className="rounded-2xl border border-white/10 bg-white/4 backdrop-blur-xl p-6 mb-6 animate-[fadeIn_0.5s_ease-out] print-card">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-xs text-white/30 font-mono">
                Commits (amber bars) · Meetings (blue bars) · Scroll horizontally · Click any day
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-[fadeIn_0.6s_ease-out]">
          {/* GitHub summary */}
          <div className="rounded-2xl border border-white/10 bg-white/4 backdrop-blur-xl p-5 print-card">
